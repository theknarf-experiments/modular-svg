...
| **Line** | connects 2 children (Refs); `source`/`target` fractional [0..1,0..1] bbox anchors, clamped-to-box endpoint defaults; stroke black sw 3, dasharray | same, ported endpoint algorithm incl. center-bias quirk (fixtures: both anchors / source only / none); dasharray via attr passthrough. Extension: **Curve** — identical endpoints rendered as a smooth cubic (control points at the midpoint of a chosen axis) | ✅ + extension |
